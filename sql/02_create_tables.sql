...
--             ) AS cassa
    ...
    cassa_data AS (
        SELECT
            ...
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento <= m1_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS novembre,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m1_end AND rfe_data_scadenza_pagamento <= m2_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS dicembre,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m2_end AND rfe_data_scadenza_pagamento <= m3_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS gennaio,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m3_end AND rfe_data_scadenza_pagamento <= m4_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS febbraio,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m4_end AND rfe_data_scadenza_pagamento <= m5_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS marzo,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m5_end AND rfe_data_scadenza_pagamento <= m6_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS aprile,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m6_end AND rfe_data_scadenza_pagamento <= m7_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS maggio,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m7_end AND rfe_data_scadenza_pagamento <= m8_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS giugno,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m8_end AND rfe_data_scadenza_pagamento <= m9_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS luglio,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m9_end AND rfe_data_scadenza_pagamento <= m10_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS agosto,
--             ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m10_end AND rfe_data_scadenza_pagamento <= m11_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS settembre,
--             ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m11_end AND rfe_data_scadenza_pagamento <= m12_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS ottobre,
...
--             ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 AND overdue_days <= 90 THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS scaduti_90gg,
            ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS scaduti_oltre,
...
--             ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m11_end AND rfe_data_scadenza_pagamento <= m12_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2),
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m10_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2),
...
--             ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 AND overdue_days <= 90 THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2),
            ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2),
...
-- ) AS cassa
    ...
    -- Future collections @ change dates
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento <= m1_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS novembre,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m1_end AND rfr_data_scadenza_pagamento <= m2_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS dicembre,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m2_end AND rfr_data_scadenza_pagamento <= m3_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS gennaio,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m3_end AND rfr_data_scadenza_pagamento <= m4_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS febbraio,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m4_end AND rfr_data_scadenza_pagamento <= m5_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS marzo,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m5_end AND rfr_data_scadenza_pagamento <= m6_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS aprile,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m6_end AND rfr_data_scadenza_pagamento <= m7_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS maggio,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m7_end AND rfr_data_scadenza_pagamento <= m8_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS giugno,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m8_end AND rfr_data_scadenza_pagamento <= m9_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS luglio,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m9_end AND rfr_data_scadenza_pagamento <= m10_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS agosto,
--     ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m10_end AND rfr_data_scadenza_pagamento <= m11_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS settembre,
--     ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m11_end AND rfr_data_scadenza_pagamento <= m12_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS ottobre,
...
--     ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 AND overdue_days <= 90 THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS scaduti_90gg,
    ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS scaduti_oltre,
...
--     ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m11_end AND rfr_data_scadenza_pagamento <= m12_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2),
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m10_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2),
...
--     ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 AND overdue_days <= 90 THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2),
    ROUND(SUM(CASE WHEN is_overdue AND overdue_days > 60 THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2),
