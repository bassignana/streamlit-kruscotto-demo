...
--             ) AS cassa
    ...
    cassa_data AS (
        SELECT
            ...
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento <= m1_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m1,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m1_end AND rfe_data_scadenza_pagamento <= m2_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m2,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m2_end AND rfe_data_scadenza_pagamento <= m3_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m3,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m3_end AND rfe_data_scadenza_pagamento <= m4_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m4,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m4_end AND rfe_data_scadenza_pagamento <= m5_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m5,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m5_end AND rfe_data_scadenza_pagamento <= m6_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m6,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m6_end AND rfe_data_scadenza_pagamento <= m7_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m7,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m7_end AND rfe_data_scadenza_pagamento <= m8_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m8,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m8_end AND rfe_data_scadenza_pagamento <= m9_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m9,
            ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m9_end AND rfe_data_scadenza_pagamento <= m10_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m10,
--             ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m10_end AND rfe_data_scadenza_pagamento <= m11_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m11,
--             ROUND(SUM(CASE WHEN NOT is_overdue AND rfe_data_scadenza_pagamento > m11_end AND rfe_data_scadenza_pagamento <= m12_end THEN rfe_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m12,
...
-- ) AS cassa
    ...
    -- Future collections @ change dates
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento <= m1_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m1,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m1_end AND rfr_data_scadenza_pagamento <= m2_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m2,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m2_end AND rfr_data_scadenza_pagamento <= m3_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m3,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m3_end AND rfr_data_scadenza_pagamento <= m4_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m4,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m4_end AND rfr_data_scadenza_pagamento <= m5_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m5,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m5_end AND rfr_data_scadenza_pagamento <= m6_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m6,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m6_end AND rfr_data_scadenza_pagamento <= m7_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m7,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m7_end AND rfr_data_scadenza_pagamento <= m8_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m8,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m8_end AND rfr_data_scadenza_pagamento <= m9_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m9,
    ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m9_end AND rfr_data_scadenza_pagamento <= m10_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m10,
--     ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m10_end AND rfr_data_scadenza_pagamento <= m11_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m11,
--     ROUND(SUM(CASE WHEN NOT is_overdue AND rfr_data_scadenza_pagamento > m11_end AND rfr_data_scadenza_pagamento <= m12_end THEN rfr_importo_pagamento_rata ELSE 0 END)::numeric, 2) AS m12,
