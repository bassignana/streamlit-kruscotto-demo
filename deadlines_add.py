    ...
    try:
        ...
        for term in terms:
            payment_term_data = {
                ...
                'importo_pagamento_rata': float(decimal_round(term['amount'])),
...
def render_payment_terms_form(supabase_client, user_id: str, invoice_data: Dict) -> None:
        ...
        with col1:
            if st.button("Pagamento Unico", use_container_width=True):
                st.session_state.current_payment_terms = [{
                    ...
                    'importo_pagamento_rata': invoice_data['importo_totale_documento'],
                    'payment_method': 'Bonifico',
                    'cash_account': 'Banca Intesa',
    ...
    if st.session_state.current_payment_terms:
        ...
        # Summary
        total_configured = sum(term['importo_pagamento_rata'] for term in st.session_state.current_payment_terms)
            ...
            with st.container():
                ...
                with col2:
                    new_amount = st.number_input(
                        "Importo (€)",
                        min_value=0.0,
                        value=max(0.0, float(term['importo_pagamento_rata'])),
                        step=0.01,
                        key=f"amount_{i}"
                    )
